...
const BridgeChains = ({showChains, setShowChains}) => {
  return (
    <section 
      className={`w-full h-full absolute inset-0 flex flex-col gap-[20px] ${showChains ? "translate-x-[0px] opacity-1" : "translate-x-[760px] opacity-0 "} transition-all duration-[1s] bg-mainAlt p-[15px] rounded-[15px]`}>
       ...
       <div className="w-full flex flex-col gap-[15px] overflow-y-auto"> 
           { networkLists.map(network => (
              <div key={network.id} 
                 className="flex-row-center w-full justify-between bg-secondary cursor-pointer text-[#fff] p-[10px] rounded-[30px] overflow-hidden transition-[background] duration-500 hover:bg-secondaryAlt hover:text-textOrange"> 
